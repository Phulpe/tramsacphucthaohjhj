...
export async function POST(request: Request) {
  ...
  if (!limit.ok) {
    ...
    const isKillSwitch = limit.reason === 'kill-switch';

    const error = isKillSwitch
      ? 'Trạm đang tạm đóng để bảo trì 🛠️'
      : isGlobal
        ? 'Hôm nay trạm đã sạc hết "pin" rồi cậu ạ 🔋'
        : 'Cậu nhắn nhanh quá, tớ cần một chút để "thở" 🍵';

    const hint = isKillSwitch
      ? 'Trạm được chủ nhân tạm khoá. Cậu quay lại sau nhé — tớ vẫn ở đây.'
      : isGlobal
